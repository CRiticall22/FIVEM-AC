AC.runPeriodically(1000, function()
    ...
    if AC.isModuleEnabled(DetectionType.PLATE) then
        local plate = GetVehicleNumberPlateText(vehicle, false)
        for _, bp in ipairs(Config.BlacklistedPlates) do
            if plate == bp then
                AC.punish(DetectionType.PLATE, "Blacklisted plate: " .. bp)
            end
        end
    end

    if AC.isModuleEnabled(DetectionType.HORN_BOOST) then
        ...
        if GetHasRocketBoost(vehicle) and not allowed[vModel] then
            if IsVehicleRocketBoostActive(vehicle) then
                AC.punish(DetectionType.HORN_BOOST, "Horn boost")
            end
        end
    end

    if AC.isModuleEnabled(DetectionType.VEHICLE_WEAPONS)
